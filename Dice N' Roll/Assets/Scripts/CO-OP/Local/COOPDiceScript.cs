...
public class COOPDiceScript : MonoBehaviour
{
    ...
    private COOPDiceScript otherDice;
    ...
    void Start()
    {
        ...
        if(Player == 2) GetComponentInChildren<MeshRenderer>().material = p2Material;
        
        foreach(COOPDiceScript dice in GameObject.FindObjectsOfType<COOPDiceScript>()) if(dice != this) otherDice = dice;
    }
        ...
        if (DiceValue > 0)
        {
            if (inGame)
            {
                if(Player == 1)
                {
                    ...
                    if (Input.GetKey(left)) Move(Row, Column - 1, Vector3.back);

                    else if (Input.GetKey(right)) Move(Row, Column + 1, Vector3.forward);

                    else if (Input.GetKey(up)) Move(Row - 1, Column, Vector3.left);

                    else if (Input.GetKey(down)) Move(Row + 1, Column, Vector3.right);
                }
                else
                {
                    if (Input.GetKey(KeyCode.LeftArrow)) Move(Row, Column - 1, Vector3.back);

                    else if (Input.GetKey(KeyCode.RightArrow)) Move(Row, Column + 1, Vector3.forward);

                    else if (Input.GetKey(KeyCode.UpArrow)) Move(Row - 1, Column, Vector3.left);

                    else if (Input.GetKey(KeyCode.DownArrow)) Move(Row + 1, Column, Vector3.right);
    ...
    {
        if (otherDice.Row == x && otherDice.Column == y) return;
            ...
            if (grid.GetTile(x, y))
            {
                ...
                switch (tileType)
                {
                    ...
                    case TileType.Teleport:
                        ...
                        if(teleportTile.DestinationCoordinates != new Vector2(otherDice.Row, otherDice.Column)) Teleport((int)teleportTile.DestinationCoordinates.x, (int)teleportTile.DestinationCoordinates.y);
    ...
    {
        GetComponentInChildren<MeshRenderer>().material = winMaterial;

        inGame = false;
        DiceValue = 0;
        hasArrived = true;

        if(otherDice.hasArrived) GameObject.FindObjectOfType<COOPUIScript>().CompleteLevel();
    }

    private void FailLevel()
    {
        GetComponentInChildren<MeshRenderer>().material = loseMaterial;

        inGame = false;
        DiceValue = 0;

        if(otherDice.inGame) otherDice.FailLevel();
        else GameObject.FindObjectOfType<COOPUIScript>().FailLevel();
    }
}
